...
from stateprep.circuit import QubitCircuit
...
class TestFidelityMaximization:
    ...
    def setup_method(self):
        ...
        self.target_state = np.array([1, 0, 0, 0], dtype=complex)  # |00⟩
        self.initial_state = np.array([0, 1, 0, 0], dtype=complex)  # |01⟩
        
    def test_fidelity_maximization_polar_decomposition(self):
    ...
    def test_fidelity_maximization_gradient_descent(self):
    ...
    def test_fidelity_maximization_invalid_method(self):
...
class TestBasisTransformation:
    ...
    def setup_method(self):
        """Set up test fixtures."""
        # Create a 2-qubit circuit with multiple gates
        self.num_qubits = 2
        pairs = [
            ((0, 1), U1UnitaryGate()),
            ((0, 1), U1UnitaryGate())
        ...
        # Multiple target and initial states
        self.target_states = [
    ...
    def test_basis_transformation_polar_decomposition(self):
        ...
        assert optimized_circuit.num_gates == self.circuit.num_gates
        
    def test_basis_transformation_gradient_descent(self):
...
class TestEnergyMinimization:
    ...
    def setup_method(self):
        ...
        self.H = scipy.sparse.csc_matrix(np.diag([1, -1, 1, -1]))
        
    def test_energy_minimization_polar_decomposition_not_implemented(self):
    ...
    def test_energy_minimization_gradient_descent(self):
        ...
        initial_energy = self.circuit.get_energy(self.H, None)
        final_energy = optimized_circuit.get_energy(self.H, None)
...
class TestGradientFunctions:
    ...
    def test_get_fidelity_gradient_basic(self):
        ...
        assert len(grads) == self.circuit.num_trainable_gates
        
        # Check gradient shapes match parameter shapes
        for i, grad in enumerate(grads):
            gate_params = self.circuit.pairs_of_indices_and_Us[i][1].params
            assert grad.shape == gate_params.shape

    def test_get_fidelity_gradient_finite_difference(self):
        ...
        original_params = self.circuit.get_concatenated_params()
        ...
        for param_idx in range(len(original_params)):
            ...
            self.circuit.set_params(params_plus)
            ...
            self.circuit.set_params(params_minus)
        ...
        self.circuit.set_params(original_params)
    ...
    def test_get_fidelity_gradient_multiple_states(self):
...
class TestEnergyGradientFiniteDifference:
    ...
    def setup_method(self):
        """Set up test fixtures."""
        # Create a simple 2-qubit circuit with trainable parameters
        self.num_qubits = 2
        pairs = [((0, 1), U1UnitaryGate())]
        self.circuit = QubitCircuit(pairs)
    ...
    def test_energy_gradient_finite_difference(self):
        ...
        # Get analytical gradients
        analytical_grads = self.circuit.get_energy_gradient(self.H, None)
        ...
        for param_idx in range(len(original_params)):
            ...
            params_plus[param_idx] += eps
            self.circuit.set_params(params_plus)
            energy_plus = self.circuit.get_energy(self.H, None)
            
            # Backward difference
            params_minus = original_params.copy()
            params_minus[param_idx] -= eps
            self.circuit.set_params(params_minus)
            energy_minus = self.circuit.get_energy(self.H, None)
